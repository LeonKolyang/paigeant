...
def test_workflows_command_lists_workflows():
    ...
    assert corr1 in output
    assert corr2 in output
...
def test_workflow_command_shows_details_and_missing():
    ...
    runner = CliRunner()
    result = runner.invoke(app, ["workflow", corr])
    assert result.exit_code == 0
    output = result.stdout
    ...
    result_missing = runner.invoke(app, ["workflow", "missing-id"])
    assert result_missing.exit_code == 1
    assert "Workflow not found" in result_missing.stdout
